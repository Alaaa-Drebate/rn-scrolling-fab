...
import { Animated, Dimensions, Keyboard, PixelRatio, Platform, Pressable, ScrollView, StyleSheet, Text, } from 'react-native';
...
const FloatingAction = (props) => {
    const { actions, size = Dimensions.get('screen').width * 0.13, isVertical, isHidden, hideOverlay, floatingIcon, floatingIconSize, onPressItem, onPressMain, rippleColor, dismissKeyboardOnPress, shadow, } = props;
     ...
     */
    const onLayoutFABContainer = (event) => {
    ...
    return (React.createElement(Animated.View, { onLayout: onLayoutFABContainer, style: [
            styles(props).floatingButtonContainer,
            { opacity: animationController },
        ] },
        !hideOverlay && (React.createElement(Animated.View, { pointerEvents: 'none', style: [styles(props).overlayStyle, overlayStyle] })),
        React.createElement(Pressable, { onPress: onPressHandler, style: ({ pressed }) => [
                styles(props).floatingButton,
                {
                    backgroundColor: Platform.OS !== 'android' && pressed
                        ? rippleColor
                        : 'transparent',
                },
            ], android_ripple: { color: rippleColor } }, floatingIcon &&
            getValidIcon(floatingIcon, size * (floatingIconSize || 0.5)) ? (getValidIcon(floatingIcon, size * (floatingIconSize || 0.5))) : (React.createElement(Animated.View, { style: closeIconStyle },
            React.createElement(Text, { style: styles(props).text }, "+")))),
        React.createElement(Animated.View, { style: [
                styles(props).expandableView,
                { [isVertical ? 'height' : 'width']: widthAnimation },
                shadow,
            ] },
            React.createElement(ScrollView, { horizontal: !isVertical, showsHorizontalScrollIndicator: false, showsVerticalScrollIndicator: false, contentContainerStyle: styles(props).expandableViewContainer }, actions.map((item, index) => (React.createElement(IconButton, { key: index, isVertical: isVertical, item: item, style: iconsStyle, onPress: onPressItem, rippleColor: rippleColor })))))));
};
/**
 * Default props for the FloatingAction component.
 */
const styles = ({ isVertical, size, tintColor, overlayColor, position, bottomSpace, horizontalSpace, floatingIconColor, }) => StyleSheet.create({
    ...
    expandableView: {
        ...
        paddingStart: isVertical || position === Position.RIGHT ? undefined : size,
    ...
    expandableViewContainer: {
        alignItems: 'center',
        justifyContent: position === Position.LEFT && !isVertical ? 'flex-end' : 'flex-start',
        flexGrow: 1,
        overflow: 'hidden',
    },
});
